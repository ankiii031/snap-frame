...
export default function EditPortfolio() {
    ...
    const handleRemoveImage = (index) => {
        ...
        if (portfolioDetails?.image) {
            ...
            updatedPortfolioDetails.image.splice(index, 1);
    ...
    const handleSubmit = () => {
        ...
        axios.put(`${host}/api/admin/updatePortfolio`, Data)
            .then((res) => {
                console.log(res)
    ...
    useEffect(()=>{
       ...
       .then((res) => {
        if(res.data){
            const portfolioimg = res.data.filter(item => item._id == id)[0]; 
            setPortfolioDetails(portfolioimg)
            
            setImages(portfolioimg.image);
    ...
    return (
        <Box sx={{marginTop:'-80px' ,marginLeft: '240px', padding: '' ,height:'auto'}}>
            ...
            <Typography variant='body1' gutterBottom>Edit Portfolio Details</Typography>
            ...
            <Box component={Paper} elevation={0} sx={{
                ...
                <Grid container spacing={2} >
                    ...
                    <TextField
                                ...
                                name="event_date"
                                size="small"
                                value={moment(portfolioDetails?.event_date).format('YYYY-MM-DD') || ''}
                                ...
                                <div style={{ marginTop: '10px' }}>
                                    {images.map((image, index) => (
                                        <div key={index} style={{ marginBottom: '10px', display: 'flex', alignItems: 'center' }}>
                                            <Typography variant="body1" style={{ marginRight: '10px' }}>{image.name ? image.name :<><img  src={`${host}/api/image/${image}`} style={{width:'100px',height:'100px'}}  width="100%" height="30vh" sx={{borderRadius:'10px'}}/></>}
</Typography>
                                            <IconButton
                                                aria-label="delete"
                                                size="small"
