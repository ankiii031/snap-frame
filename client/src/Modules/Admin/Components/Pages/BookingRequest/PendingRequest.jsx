...
export default function PendingRequest() {
    ...
    useEffect(() => {
        axios.get(`${host}/api/user/getAllBookingDetails`)
            .then((res) => {
                if (res.data) {
                    const pendingData = res.data.filter((i) => i.status == 'pending')
    ...
    return (
        <Box sx={{marginTop:'-80px' ,marginLeft: '240px', padding: '' ,height:'auto'}}>
            {bookingDetails.length > 0 &&
                ...
                <h2 style={{ color: '#212529' }}>Manage Pending Booking Request</h2>
            }
                ...
                {bookingDetails.length > 0 ?
                            ...
                            <TableBody>
                                ...
                                {bookingDetails.slice().reverse().map((row) => (
                                    <StyledTableRow key={row._id}>
                                        ...
                                        <StyledTableCell >
                                            {moment(row.b_date).format("MMM Do YYYY")}
                                        </StyledTableCell>
                                        <StyledTableCell >
                                            <Box sx={{display:'flex',flexDirection:'column',gap:'4px'}}>
                                                <Button variant='contained' onClick={() => handleRequest(row._id,'confirmed')} size='small' color='success' startIcon={<CheckCircleOutlineOutlinedIcon />}>
                                                    Accept
                                                </Button>
                                                <Button variant='contained' onClick={() => handleRequest(row._id,'declined')} size='small' color='error' startIcon={<CancelOutlinedIcon />}>
                                                    Reject
                                                </Button>
                                            </Box>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', height: '50vh', mt: 13 }}>
                        ...
                        <Grid item xs={12} lg={12} justifyContent="center" alignItems="center">
                            ...
                            <Typography variant='body1' gutterBottom sx={{ color: 'blue' }}>No Pending Request Found!</Typography>
                        </Grid>
                    </Box>
                }
            </Box>
