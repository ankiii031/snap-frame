...
export default function PaymentDetails() {
    ...
    useEffect(() => {
        axios.get(`${host}/api/user/getAllBookingDetails`)
            .then((res) => {
                if (res.data) {
                    const pendingData = res.data.filter((i) => i.status == 'confirmed')
    ...
    const handleRequest = (id) => {
        ...
        axios.put(`${host}/api/admin/updateRequest/${id}`, { status: 'confirmed' })
            .then((res) => {
                if (res.data) {
    ...
    return (
        <Box sx={{marginTop:'-80px' ,marginLeft: '240px', padding: '' ,height:'auto'}}>
            ...
            <h2 style={{ color: '#212529' }}>View Payment Details</h2>
                }
                ...
                {bookingDetails.length > 0 ?
                                    ...
                                    <StyledTableRow key={row._id}>
                                        ...
                                        <StyledTableCell >
                                            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                                                ...
                                                <Typography variant='body2' color='text.secondary' sx={{ display: 'flex', alignItems: 'center', gap: '2px',color:'green' }} >{row?.s_id.category.title}</Typography>
                                        ...
                                        <StyledTableCell >
                                            ...
                                            {row?.paymentStatus == 'pending' &&
                                                    <Chip icon={<PendingOutlined />} label="Pending" size='small' color='warning' />
                                                }
                                                {row?.paymentStatus == 'paid' &&
                                                    <Chip icon={<CheckCircleOutlineOutlinedIcon />} label="Paid" size='small' color='success' />
                                                }
                                            </Box>
                                        </StyledTableCell>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center',height:'50vh',mt:13 }}>
                        ...
                        <Grid item xs={12} lg={12} justifyContent="center" alignItems="center">
                            ...
                            <Typography variant='body1' gutterBottom sx={{ color: 'blue' }}>No Payment Details Found!</Typography>
                        </Grid>
                    </Box>
                }
            </Box>
